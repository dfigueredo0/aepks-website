...
/* === your post-processing wrapper (unchanged) === */
...
function DitheredWaves({
  ...
  const mouseRef = useRef(new THREE.Vector2(-1e5, -1e5)); // off-screen default
  ...
  const uniforms = useRef({
    ...
    mousePos: new THREE.Uniform(new THREE.Vector2(-1e5, -1e5)),
    enableMouseInteraction: new THREE.Uniform(enableMouseInteraction ? 1 : 0),
    mouseRadius: new THREE.Uniform(mouseRadius),
  });

  // keep resolution (in physical px)
  useEffect(() => {
  ...
  // ✅ global pointer tracking: works even with canvas behind content
  useEffect(() => {
    ...
    const onLeave = () => mouseRef.current.set(-1e5, -1e5);
  ...
  // frame updates
  useFrame(({ clock, size: sz, gl: r }) => {
    if (!disableAnimation) uniforms.current.time.value = clock.getElapsedTime();
    const dpr = r.getPixelRatio();
    uniforms.current.resolution.value.set(
      Math.floor(sz.width * dpr),
      Math.floor(sz.height * dpr)
    );
